...
**Weekly label format**: Weekly buckets are labelled by their week-of-month based on the Monday of each week. For example, if the Monday of a week falls on 30 March, it is labelled "Week 5 - Mar 2026" — even if trips from that week extend into April. This matches how calendar weeks are typically communicated.
...
The projected **annual** savings if the fleet's idling ratio is reduced by 25% from its current level. This figure is based on the vehicles currently included in the analysis.
...
The sub-text shows the vehicle count and the resulting idling rate change across two lines (e.g., "Annually based on 59 vehicles" / "22.1% → 16.6%").

### Scaled Annual Savings

The same 25% reduction savings, scaled up to your **Estimated Total Vehicles** figure from the dashboard. This is the number most relevant for sales conversations — it projects the full-fleet ROI from reducing idling.
...
If no estimated fleet size is set (or it is not larger than the current fleet), this card shows the same value as Savings at 25% Reduction.

The sub-text shows the projected fleet size and reduction scenario across two lines (e.g., "Projected for 100 vehicles" / "At 25% reduction").

### Savings to Reach 10%

The projected **annual** savings if the fleet reduces its idling rate all the way to the **10% industry benchmark** (top quartile). This is the maximum realistic improvement scenario and is also scaled to the estimated fleet size.

**Calculation**:
```
Ideal Savings = Annual Idle Cost × (1 − 10 / Idling%)
Scaled Ideal Savings = Ideal Savings × Scale Factor
```

This card only applies when the fleet's idling rate is **above 10%**. If the fleet is already at or below 10%, this card is replaced by a **Top Quartile Fleet** recognition card indicating the fleet has already achieved the benchmark.

The sub-text shows the projected fleet size and the target idling rate change across two lines (e.g., "Projected for 100 vehicles" / "28.7% → 10%").
...
The tool annualizes savings based on the analysis period length, then applies optional fleet scaling. All dollar values on the report use this flow:
...
Savings at 25% Reduction = Annual Idle Cost × 0.25
Scale Factor = Estimated Vehicles / Current Vehicles  (≥ 1; 1 if no scaling)
Scaled Annual Savings = Savings at 25% × Scale Factor

Idling % After 25% Reduction = Current Idling% × 0.75
Savings to Reach 10% = Annual Idle Cost × (1 − 10 / Idling%)  [only when Idling% > 10]
Scaled Savings to Reach 10% = Savings to Reach 10% × Scale Factor
```
...
The 10% idling benchmark used in the tool is based on industry data for commercial fleet performance:

| Tier | Rate | Basis |
|------|------|-------|
| Top Quartile | ≤ 10% | Best-in-class fleets with active idling coaching programmes |
| Industry Average | ~20% | Standard baseline for regional delivery and mixed-use commercial fleets before aggressive coaching |

These benchmarks are used to contextualise the Savings to Reach 10% scenario and to colour-code the idling cards on the report. They are directional references — exact benchmarks vary by industry segment, region, and fleet type. Geotab's product team is continuing to validate these figures with real-world fleet data.
